...
Original file is located at
    https://colab.research.google.com/drive/1gQ294FHPxI0rJOqpsNQKNTOBJ4vS9R5V
"""
...
def predict_example(x, dec_tree):
    """
    ...
    # INSERT YOUR CODE HERE. NOTE: THIS IS A RECURSIVE FUNCTION.
    if type(dec_tree) != dict:
        return dec_tree
    
    for k in dec_tree.keys():
        i = k[0]
        val = k[1]
        flg = k[2]
        
        if x[i] == val and flg == True:
            next_lev = predict_example(x, dec_tree.get(k))
        if x[i] != val and flg == False:
            next_lev = predict_example(x, dec_tree.get(k))
